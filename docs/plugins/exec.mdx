...
By default, the read-only `KUBECONFIG` is assigned. For enabling commands that require create, update or delete rules, you need to create specific (Cluster)Role and (Cluster)RoleBinding and reference it from plugin's RBAC configuration. To learn more refer to the [RBAC section](../rbac.md).
...
:::tip
For convenience, you can configure a Botkube alias to simplify running `flux` commands, according to the [Alias configuration](../../configuration/alias) document. Later, you can simply type `@Botkube flux get sources git`.
:::
